...
/**
 * The codec support matrix from CMakeLists.txt: each fixture is a real 3-page TIFF
 * (single-page for the CCITT one, which needs genuinely bilevel source data), one compression
 * scheme per file, generated fresh for this suite (see generate_fixtures.py). Supported codecs must decode
 * every page cleanly; codecs this build deliberately doesn't compile in must fail loudly and
 * specifically from {@code render()} -- never from {@code openPage()}, since the compression tag
 * is just metadata until a codec is actually invoked to decode pixels.
 */
...
public class TiffRendererCodecTest {
    ...
    /**
     * libtiff doesn't reject an unsupported compression scheme at {@code TIFFClientOpen}/
     * directory-read time -- the compression tag is just metadata, so {@link
     * TiffRenderer#getPageCount()} and {@link TiffRenderer#openPage} both succeed. The codec is
     * only actually invoked, and only then fails, once {@link TiffRenderer.Page#render} calls
     * {@code TIFFReadRGBAImageOriented}.
     */
