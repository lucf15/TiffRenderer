...
public class TiffRendererRetainRasterTest {
     ...
     * not stale/garbage data from a freed or mis-cached raster.
     */
    @Test
    ...
    /**
     * A codec this build can't decode must fail loudly from retainRaster() itself, not silently
     * succeed and misbehave on the next render() -- mirrors TiffRendererCodecTest's finding that
     * the codec is only actually invoked at decode time.
     */
    @Test
    ...
    /**
     * Closing a page that opted into retainRaster() must not leak the cached raster into the
     * next page opened on the same renderer.
     */
    @Test
    public void releasedOnPageClose_doesNotLeakIntoNextPage() throws IOException {
        try (ParcelFileDescriptor pfd = TestAssets.open("varying_page_dimensions.tif");
            ...
            // Page 2 is a different size (20x15 vs 10x10) -- if the cache leaked, this would
            // either crash (dimension mismatch against a stale raster) or misdecode.
