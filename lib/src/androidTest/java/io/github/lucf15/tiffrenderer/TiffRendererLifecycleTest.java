...
/**
 * {@link TiffRenderer}/{@link TiffRenderer.Page}'s state machine: construction, {@code
 * openPage()}/{@code close()} preconditions, and the "one page open at a time" invariant. Render
 * correctness lives in {@link TiffRendererRenderTest}; codec support lives in
 * {@link TiffRendererCodecTest}.
 */
...
public class TiffRendererLifecycleTest {
    ...
    public void renderer_finalizeWithoutClose_doesNotThrow() throws Throwable {
        // Callable directly: this test is in the same package as TiffRenderer's protected
        // finalize().
        TiffRenderer renderer = new TiffRenderer(TestAssets.open("single_page_rgb.tif"));
        renderer.finalize();
        assertTrue(true);
